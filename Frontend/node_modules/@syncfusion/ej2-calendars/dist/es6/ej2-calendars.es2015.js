...
let CalendarBase = class CalendarBase extends Component {
    ...
    renderDays(currentDate, value, multiSelection, values, isTodayDate) {
        ...
        for (let day = 0; day < cellsCount; ++day) {
            ...
            const dateFormatOptions = { type: 'dateTime', skeleton: isBlazor() ? 'D' : 'full' };
            ...
            const title = this.globalize.formatDate(localDate, { type: 'date', skeleton: isBlazor() ? 'D' : 'full' });
                ...
                if (multiSelection) {
                    if (!isNullOrUndefined(values) && values.length > 0) {
                        for (let index = 0; index < values.length; index++) {
                            const localDateString = +new Date(this.globalize.formatDate(argument.date, { type: 'date', skeleton: isBlazor() ? 'd' : 'yMd' }));
                            const tempDateString = +new Date(this.globalize.formatDate(values[index], { type: 'date', skeleton: isBlazor() ? 'd' : 'yMd' }));
            ...
            if (multiSelection && !isNullOrUndefined(values) && !disabledCls) {
                for (let tempValue = 0; tempValue < values.length; tempValue++) {
                    ...
                    const formatOptions = { format: this.getFromatStringValue(), type: 'date', skeleton: 'short', calendar: type };
    ...
    renderDecades(e, value) {
        ...
        const endHdrYr = this.globalize.formatDate(endYr, { format: isBlazor() ? 'yyyy' : null, type: 'dateTime', skeleton: 'y' });
    ...
    dayCell(localDate) {
        const type = (this.calendarMode === 'Gregorian') ? 'gregorian' : 'islamic';
        const dateFormatOptions = { skeleton: isBlazor() ? 'F' : 'full', type: 'dateTime', calendar: type };
        const date = this.globalize.parseDate(this.globalize.formatDate(localDate, dateFormatOptions), dateFormatOptions);
    ...
    validateValues(multiSelection, values) {
        if (multiSelection && !isNullOrUndefined(values) && values.length > 0) {
            ...
            for (let skipIndex = 0; skipIndex < copyValues.length; skipIndex++) {
                ...
                const minFormatOption = { type: 'date', skeleton: isBlazor() ? 'd' : 'yMd', calendar: type };
                const minStringValue = this.globalize.formatDate(this.min, minFormatOption);
                const minString = minStringValue;
                const maxFormatOption = { type: 'date', skeleton: isBlazor() ? 'd' : 'yMd', calendar: type };
    ...
    getCultureObjects(ld, c) {
        const gregorianFormat = (isBlazor() ? '.dates.days.' :
            '.dates.calendars.gregorian.days.format.') + this.dayHeaderFormat.toLowerCase();
        const islamicFormat = (isBlazor() ? '.dates.days.' :
    ...
    getIdValue(e, element) {
        ...
        const type = (this.calendarMode === 'Gregorian') ? 'gregorian' : 'islamic';
        const dateFormatOptions = { type: 'dateTime', skeleton: isBlazor() ? 'F' : 'full', calendar: type };
        // eslint-disable-next-line radix
...
let DateRangePicker = class DateRangePicker extends CalendarBase {
    ...
    updateHiddenInput() {
        if (this.firstHiddenChild && this.secondHiddenChild) {
            const format = { type: 'datetime', skeleton: isBlazor() ? 'd' : 'yMd' };
    ...
    checkHtmlAttributes(isDynamic) {
        ...
        const attributes$$1 = isDynamic ? isNullOrUndefined(this.htmlAttributes) ? [] : Object.keys(this.htmlAttributes) :
            ['startDate', 'endDate', 'minDays', 'maxDays', 'min', 'max', 'disabled', 'readonly', 'style', 'name', 'placeholder',
                'type', 'value'];
        const format = { format: this.formatString, type: 'date', skeleton: isBlazor() ? 'd' : 'yMd' };
        for (const prop of attributes$$1) {
            if (!isNullOrUndefined(this.inputElement.getAttribute(prop))) {
                switch (prop) {
                    case 'disabled':
    ...
    inputBlurHandler(e) {
        ...
        if (!this.preventBlur) {
            ...
            if (!isNullOrUndefined(value) && value.trim() !== '') {
                ...
                if (range.length > 1) {
                    this.invalidValueString = null;
                    const dateOptions = { format: this.formatString, type: 'date', skeleton: isBlazor() ? 'd' : 'yMd' };
    ...
    updateHeader() {
        const format = { type: 'date', skeleton: isBlazor() ? 'D' : 'yMMMd' };
        if (!isNullOrUndefined(this.endValue) && !isNullOrUndefined(this.startValue)) {
    ...
    addSelectedAttributes(ele, date, isStartDate, sameDate) {
        if (ele) {
            const title = this.globalize.formatDate(date, { type: 'date', skeleton: isBlazor() ? 'D' : 'full' });
    ...
    updateInput() {
        if (!isNullOrUndefined(this.endValue) && !isNullOrUndefined(this.startValue)) {
            const formatOption = { format: this.formatString, type: 'date', skeleton: isBlazor() ? 'd' : 'yMd' };
...
let TimePicker = class TimePicker extends Component {
    ...
    getCldrDateTimeFormat() {
        const culture = new Internationalization(this.locale);
        let cldrTime;
        const dateFormat = culture.getDatePattern({ skeleton: isBlazor() ? 'd' : 'yMd' });
    ...
    getMeridianText() {
        ...
        if (this.locale === 'en' || this.locale === 'en-US') {
            ...
            const gregorianFormat = (isBlazor() ? '.dates.dayPeriods.abbreviated' :
    ...
    setScrollTo() {
        ...
        if (!isNullOrUndefined(this.popupWrapper)) {
            ...
            if (items.length) {
                const initialTime = this.isBlazorServer ? new Date(new Date().toDateString() + ' ' +
                    this.blazorTimeCollections[0]).setMilliseconds(0) : this.timeCollections[0];
                const scrollTime = this.isBlazorServer ? new Date(new Date().toDateString() + ' ' +
    ...
    findNextElement(event) {
        ...
        const value = isNullOrUndefined(this.valueWithMinutes) || this.isBlazorServer ? this.createDateObj(textVal) :
            this.getDateObject(this.valueWithMinutes);
        let timeVal = null;
        const count = this.liCollections.length;
        const collections = this.isBlazorServer ? (isNullOrUndefined(this.blazorTimeCollections) ? [] :
    ...
    keyHandler(event) {
        ...
        const count = this.isBlazorServer ? (isNullOrUndefined(this.blazorTimeCollections) ? 0 :
...
class MaskedDateTime {
        ...
        function formatValueSpecifier(formattext) {
            ...
            switch (formattext) {
                ...
                case 'aa':
                    ...
                    break;
                case 'zzz':
                    let offset = proxy.value.getTimezoneOffset();
                    let offsettime;
                    isSymbol = offset < 0;
                ...
                case 'z':
                case 'zz':
                    let offsetValue = proxy.value.getTimezoneOffset() / 60;
    ...
    dateAlteration(isDecrement) {
        ...
        let newDateValue = new Date(this.value.getFullYear(), this.value.getMonth(), this.value.getDate(), this.value.getHours(), this.value.getMinutes(), this.value.getSeconds());
...
export { CalendarBase, Calendar, Islamic, DatePicker, Presets, DateRangePicker, TimePickerBase, TimePicker, DateTimePicker, MaskedDateTime };
